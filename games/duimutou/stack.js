...
function stack_init() {
    ...
    stmargin[sts] = 0;
    stdir = Math.log(sts + 1) * 2;
    $('level').innerHTML = Math.floor(sts / 10) + 1;
    sttype[sts] = Math.floor(Math.random() * 4) + 1;
    if (Math.random() > .5) {
        stdir *= -1;
        stmargin[sts] = saw - stwidth[sts];
    }
    tmp = document.createElement('div');
    tmp.id = 'stp_' + sts;
...
function handleclick() {
    ...
    if (stmargin[sts] < stmargin[sts - 1]) {
        stwidth[sts] -= stmargin[sts - 1] - stmargin[sts];
        stmargin[sts] = stmargin[sts - 1];
        hcf = 'left';
    }
    if (stmargin[sts] + stwidth[sts] > stmargin[sts - 1] + stwidth[sts - 1]) {
        stwidth[sts] -= stmargin[sts] + stwidth[sts] - stmargin[sts - 1] - stwidth[sts - 1];
        hcf = 'right';
    }
...
function cutaronk(crid, crtype, crstart, crstays, crfrom) {
    ...
    if (crfrom != '') {
        ...
        document.getElementById(crid).style.marginTop = (sah - (sts + 1) * (logh - 4)) + 'px';
        document.getElementById(crid).childNodes[0].style.zIndex = ((sts % 2) ? (13) : (16));
        stack_init();
    }
}
...
var DFW = {
    appId: "",
    TLImg: "http://g.ohayo.cyou/games/duimutou/icon.png",
    url: "http://g.ohayo.cyou/games/duimutou/",
    title: "堆木头，简单的不得了，好玩的停不下",
    desc: "玩法简单，但是玩的是技巧，反应，没有最高只有更高，来挑战下--关注游戏中心:@KarlesSlaveBot"
};
...
imgUrl = 'http://g.ohayo.cyou/games/duimutou/i/icon.png';
lineLink = 'http://g.ohayo.cyou/games/duimutou';
